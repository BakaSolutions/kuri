...
const config = { // Bootleg, later this'll be fetched from server
	fileUpload: {
		maxFiles: 3,
...
function createElement (tagName, attributes = {}){
	...
	for (let attr in attributes) element[attr] = attributes[attr];
	return element
}
...
function loadPage(URL) {
	...
	XHR.onload = () => {
		...
		const MASTER_CSS = /(<link rel="stylesheet" id="mainStylesheet" href=".+">)/.exec(XHR.response);
		document.querySelector('#mainStylesheet').outerHTML = MASTER_CSS[1];
